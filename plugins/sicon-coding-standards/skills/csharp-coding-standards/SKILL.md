---
name: csharp-coding-standards
description: Apply Sicon C# naming conventions, coding standards, and best practices when writing or reviewing C# code.
---

# C# coding standards

Use when editing or reviewing `.cs` files, or when the user asks about Sicon C# style.

## Instructions

1. Read `../../content/csharp-coding-standards.md`.
2. Apply those naming, style, structure, practice, and Sage conventions.
3. Prefer that document over memory; do not invent an alternate style guide.
